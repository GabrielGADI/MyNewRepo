...
class Creature(ABC): # astfel este definita o functie abstracta
    @abstractmethod
    def eat(self):
        pass
...
def get_all_domestic_animals(animals):
    li = []
    for x in animals:
        if isinstance(x, DomesticAnimal): # o functie de sine statatoare (isinstance)
            # if type(x) == DomesticAnimal: -> daca vrem doar obiecte strict de tipul DomesticAnimal
            li.append(x) # cand gasim animal domestic se vor pune in lista
    return li
